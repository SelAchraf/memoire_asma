    ...
    <?php
        ...
        if(isset($_REQUEST['courseUpdtbtn'])){
            if(($_REQUEST['course_name'] == "") || ($_REQUEST['course_lang'] == "") || ($_REQUEST['course_tut'] == "")){
                ...
                $sql= "UPDATE course SET course_id='$course_id',course_name='$course_name',course_lang='$course_lang',course_tut='$course_tut',course_img='$img_folder'";
                if($conn->query($sql) ==TRUE){
                    $msg = '<div class="alert alert-success col-sm-6 ml-5 mt-2 role="alert"
                    ">Update sccessfully</div>';
                }
                else{
    ...
    <div class="col-sm-9 mt-5 mx-3 ml-3 float-xl-right jumbotron " >
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control" id="course_lang" name="course_lang" value="<?php if(isset($row['course_lang'])){ echo $row['course_lang'];} ?>">
            </div>
            <div class="form-group">
                <label for="course_tut">Course Tutorial</label>
                <input type="text" class="form-control" id="course_tut" name="course_tut" value="<?php if(isset($row['course_tut'])){ echo $row['course_tut'];} ?>">
            </div>
            <div class="form-group">
                <label for="course_img">Course Image    </label>
                <img src="<?php if(isset($row['course_img'])) { echo $row['course_img'];}?>" alt="" class="img-thumbnail"  >
                <br><br><input type="file" class="form-control-file" id="course_img" name="course_img">
            </div>
